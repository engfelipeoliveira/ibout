...
public class MainServiceImpl implements MainService {
	...
	@Value("${msg.error.validation.parameters.not.found}")
	private String MSG_ERROR_VALID_PARAMS_NOT_FOUND;

	@Value("${msg.error.validation.mapping.not.found}")
	private String MSG_ERROR_VALID_MAP_NOT_FOUND;
	...
	public void execute(String tokenClient) throws Exception {
		...
		if (parameter == null) {
			ErrorJob error = ErrorJob.builder().job(job).description(MSG_ERROR_VALID_PARAMS_NOT_FOUND).build();
			...
			if (listError.isEmpty()) {
				...
				if (mapping == null) {
					ErrorJob error = ErrorJob.builder().job(job).description(MSG_ERROR_VALID_MAP_NOT_FOUND).build();
					job = job.toBuilder().endTime(LocalDateTime.now()).status("ERRO").errors(Sets.newHashSet(error))
							.build();
					LOG.info("Processo finalizado com erro");
